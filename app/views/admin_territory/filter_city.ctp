...
<?php foreach($ArSearchCity as $SearchCity) {?>
	<li id="city_class_<?php echo $SearchCity['City']['id'];?>" <?php if(in_array($SearchCity['City']['id'], $city_ids)){?> class="active" <?php } ?>><a href="javascript:void(0)" onclick="select_city(<?php echo $SearchCity['City']['id'];?>)"><?php echo $SearchCity['City']['name'];?></a></li>
<?php } ?>
</ul>
</div>
<?php }else{
