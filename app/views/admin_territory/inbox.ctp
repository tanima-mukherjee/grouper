    ...
    <script type="text/javascript">
      jQuery(document).ready(function($) {
        ...
        $('#deleteCategory').on('click', function() {
          var confirm = window.confirm('Are you sure you want to delete the message(es)?');
          if(confirm) {
            ...
            $.ajax({
              type: 'GET',
              url: '<?php echo $this->webroot; ?>admin_territory/delete_message',
          ...
          <div class="contentinner content-dashboard">            	                
            <div class="row-fluid">
              ...
              <table class="table table-bordered" id="dyntable">
                ...
                <tbody>
                  <?php if(!empty($territory_messages)) : foreach($territory_messages as $message) : ?>
                    <tr class="gradeX">
                          <td class="aligncenter">
                            <span class="center">
                              <input type="checkbox" class="check checkindividual" value="<?php echo $message['TerritoryMessage']['id']; ?>" />
                            </span>
                          </td>
                          <td><?php echo (strlen($message['TerritoryMessage']['message']) > 100) ? substr($message['TerritoryMessage']['message'],0,100).'...' : $message['TerritoryMessage']['message']; ?></td>
                          <td><?php echo date('Y-m-d H:i A',strtotime($message['TerritoryMessage']['created'])); ?></td>

                          <!-- <td><a href="javascript:void(0)" onClick="change_status(<?php echo $message['TerritoryMessage']['id']; ?>, '<?php echo $message['TerritoryMessage']['status']; ?>');"><?php if($message['TerritoryMessage']['status'] == '1') echo 'Active'; else echo 'Inactive'; ?></a></td>
                          <td> -->
                          <td><a href="<?php echo $this->webroot.'admin_territory/view_message/'.$message['TerritoryMessage']['id'];?>">View</a></td>
                      </td>
                    </td>
					         </tr>
