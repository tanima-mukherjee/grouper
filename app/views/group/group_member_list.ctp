...
<script>
  ...
  function make_owner(sender_id,receiver_id,group_id,member_mode,k, group_user_id){
		...
		jQuery.ajax({
          ...
          if(response == '1'){
            ...
            jQuery('#group_owner_previlige'+k).html('<button  type="button" class="owner-btn" id="" onClick="remove_owner('+sender_id+','+receiver_id+','+group_id+','+'\''+member_mode_old+'\''+','+k+','+group_user_id+')" >Remove Owner</button>');
			jQuery('#remove_member'+k).html('');
			jQuery('#remove_member'+k).hide();
			jQuery('#sms_send_'+k).html('');
			jQuery('#sms_send_'+k).hide();
          }
          }
      });
  }
  
  function remove_owner(sender_id,receiver_id,group_id,member_mode,k,group_user_id){
  		var member_mode_old= jQuery('#mode_member'+k).html();
		...
		jQuery.ajax({
          ...
          if(response == '1'){
		  	jQuery('#mode_member'+k).html(member_mode);
            jQuery('#group_owner_previlige'+k).html('<button  type="button" class="owner-btn" id="" onClick="make_owner('+sender_id+','+receiver_id+','+group_id+','+'\''+member_mode_old+'\''+','+k+','+group_user_id+')" >Make Owner</button>');
			jQuery('#remove_member'+k).show();
			jQuery('#remove_member'+k).html('<button  type="button" class="owner-btn" id="" onClick="remove_member('+sender_id+','+receiver_id+','+group_id+','+k+')" >Remove Member</button>');
			jQuery('#sms_send_'+k).show();
			jQuery('#sms_send_'+k).html('<button  type="button" class="owner-btn" id="" onClick="posting_status('+group_user_id+','+group_id+','+k+','+'1'+')" >Disallow Post</button>');
  ...
  function posting_status(group_user_id,group_id,k,status){
     // var member_mode_old= jQuery('#mode_member'+k).html();
//       alert(group_user_id);
//	   alert(group_id);
//	   alert(k);
...
//	   exit();
	   if(status=='0'){
	   		var confirm = window.confirm("Are you sure to allow user to post ?");
	   }
	   else{
	   		var confirm = window.confirm("Are you sure to disallow user to post ?");
	   }
       ...
       if(confirm){
        ...
        jQuery.ajax({
          ...
          type: "GET",
          url: WEBROOT+"group/update_message_posting_status",
            ...
            //alert(response);
              if(jQuery.trim(response) == 1)
              {
                  if(status == '1')
                  {
                    post_status = '0';
                    txt_show = 'Allow Post';
					//alert(post_status+'==='+txt_show);
                  }else
                  {
                    post_status = '1';
                    txt_show = 'Disallow Post';
                  }
                  //alert(txt_show+','+post_status);
                  jQuery('#sms_send_'+k).html('<button  type="button" class="owner-btn" id="" onClick="posting_status('+group_user_id+','+group_id+','+k+','+post_status+')" >'+txt_show+'</button>');
...
<body>
    ...
    //echo '<pre>';exit;
	  ...
	  <?php if($session_user_id == $created_by && $session_user_id!=$list['user_id']){ ?>
			...
			<?php if($list['user_type']=='O'){ ?>
				<button  type="button" class="owner-btn" id="" onClick="remove_owner(<?php echo $session_user_id; ?>,<?php  echo $list['user_id']; ?>,<?php  echo $list['group_id']; ?>, '<?php echo ucfirst($list['member_mode']);?>', <?php  echo $k; ?>, <?php echo $list['id']; ?>)" >Remove Owner</button>
			<?php }
				else{
			?>
				<button  type="button" class="owner-btn" id="" onClick="make_owner(<?php echo $session_user_id; ?>,<?php  echo $list['user_id']; ?>,<?php  echo $list['group_id']; ?>, 'Owner', <?php  echo $k; ?>, <?php echo $list['id']; ?>)" >Make Owner</button>
			<?php	
				}?>
      </div>
		  <?php } ?>

	<div id="remove_member<?php echo $k;?>">
      ...
      <?php if($list['group_type']=='F' && $list['user_type']=='M' && $is_owner==1){ 
       if($list['can_post_topic'] == '0'){ ?>
        
        <button type="button" class="owner-btn" id="" onClick="posting_status(<?php echo $list['id']; ?>,<?php  echo $list['group_id']; ?>, <?php  echo $k?>,'0')" >Allow Post</button>
        
        <?php }else{ ?>

        <button type="button" class="owner-btn" id="" onClick="posting_status(<?php echo $list['id']; ?>,<?php  echo $list['group_id']; ?>, <?php  echo $k?>,'1')" >Disallow Post</button>
