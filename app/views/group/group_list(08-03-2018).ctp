...
<script >
  $(document).ready( function() {
      ...
      var validator = $("#group_formm").submit(function() {
...
<!-- Sign up Modal -->
<?php 
	if($category_details['Category']['POG_status']=='1'){
		$title= 'Create Public Organization Groop';
		$url= 'add_public_organization_category_group';
	}
	else{
		$title= 'Create Business Groop';
		$url= 'add_business_category_group';
	}
?>
	 ...
	 <div class="modal-dialog signup-model">
	 	<div class="modal-content">
			...
			<div class="modal-body group-modal-body">
				<form action="<?php echo $this->webroot?>group/<?php echo $url; ?>" method="post" id="group_formm" name="group_formm" enctype="multipart/form-data">
    ...
    <div class="container">
	  ...
	  <?php if ($this->Session->read('userData')) { ?>
		  <div class="page-right-nav">
			<ul>	
			  ...
			  <li>
				<a href="javascript:void(0)" data-toggle="modal" data-target="#creategrouppage" class="create-link fright">Create Groop <i class="fa create-icon"></i></a>
			  </li>
		  </ul>
		  </div>
	  <?php } ?>   
      ...
      <div class="category-heading-details">
        ...
        <div class="category-des-info">
          <h4><?php echo ucfirst(stripslashes($category_details['Category']['title']))?></h4>
