...
jQuery(document).ready( function() {
     var validator = $("#join_request_form").submit(function() {
      ...
      errorPlacement: function(label, element) {
       ...
       messages: {
	   	  "push[]": {        
			required:" Please check atleast one Owner"
		  }, 
		  message : {        
...
$("#all_owners").change(function(){  //"select all" change 
...
$('.checkindividual').change(function(){ //".checkbox" change 
    ...
    if(this.checked == false){ //if this item is unchecked
        $("#all_owners")[0].checked = false; //change "select all" checked status to false
    }
    
    //check "select all" if all checkbox items are checked
    if ($('.checkindividual:checked').length == $('.checkindividual').length ){ 
        $("#all_owners")[0].checked = true; //change "select all" checked status to true
    }
});
}
</script>
...
<body>
	<div class="user-dialog">
     <div class="modal-header">        
		...
		<div class="select-all-checkbox">
			<!-- <input type="checkbox" /> -->
      		<input type="checkbox" name="all_owners" id="all_owners" onClick="check_owners();">
			<label>Select All</label>						
		</div>
   	 	<?php } ?>
      </div>
 	  <form action="<?php echo $this->webroot?>group/push_message_owner_sent" method="post" id="join_request_form" name="join_request_form" target="_parent">  
      ...
      <li>
        ...
        <div class="view-userdes">
          <div class="align-center">
            <span class="user-request-display"><?php echo ucfirst($list['user_name']) ;?></span>
            <div class="actions actions-right action-checkbox" >
				  ...
				  <label>&nbsp;</label>
					...
					<label generated="true" class="error" style="display:none;"> Please check atleast one owner</label>
            </div>
	...
	<div class="message-bottombg">
		<p style="color:#FF0000; font-size:14px; display:none;" id="err_check1">Please check minimum one owner !!!</p>
