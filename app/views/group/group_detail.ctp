...
<script >
  $(document).ready( function() {
      ...
      var validator = $("#sub_group_form").submit(function() {
...
function ChangeGroup(group_type){
  //alert(group_type);
	...
	if(group_type == 'B' || group_type == 'PO')
	{
	  jQuery("#category").show();
	  if(group_type=='B'){
	  	var confirm_msg = confirm("When you make a group as Business group, all of it's events will be made public");
		
		if (confirm_msg == true) {
			jQuery("#category_id_POG").hide();
			jQuery("#category_id_B").show();
		}
		else{
			jQuery("#new_group_type").val('PO');
		}	
	  }
	  else if(group_type=='PO'){
			jQuery("#category_id_B").hide();
			jQuery("#category_id_POG").show();
	  }
	}
...
function select_category(category_id){
	//var cat= parseInt(jQuery("#category_id").val());
	//alert(cat);
	jQuery("#alok").val(category_id);
}
...
<script>
  function showMyImageSub(fileInput) {
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      var img = document.getElementById("thumbnil100");
      img.file = file;
...
var confirm = window.confirm('Are you sure you want to delete that group?');
if(confirm) {
  jQuery.ajax({
	type: 'POST',
	url: '<?php echo $this->webroot; ?>group/delete_group',
 ...
 <style>
   ...
   .upcoming-events-list ul li .no-deal-amount{ overflow:hidden;}
 </style>
 <!-- Compose Modal -->
...
<div class="modal fade" id="editgroup" role="dialog">
    ...
    <div class="modal-header signup-header">
		<button type="button" class="close" data-dismiss="modal">×</button>
		<h4 class="modal-title">Edit Groop</h4>
    </div>
   ...
   <div class="modal-body group-modal-body">
	   <form action="<?php echo $this->webroot.'group/edit_group/'.$group_details['Group']['id'];?>" method="post" id="edit_group_form" name="edit_group_form" enctype="multipart/form-data">  
		...
		<input type="hidden" name="alok" id="alok" value="<?php echo $group_details['Group']['category_id'];?>" />
		...
		<div class="pop-createg">
			...
			?>
				<img id="thumbnil8" src="<?php echo $this->webroot;?>sub_group_images/<?php echo $group_details['Group']['icon'];?>" alt="">
    ...
    <div class="form-group">
      <label class="label-field">Groop Type</label>
      					<select class="form-control" name="new_group_type" id="new_group_type" onChange="ChangeGroup(this.value);" >
                        <option value="">Select One</option>
						<option value="PO" <?php if($group_details['Group']['group_type']=="PO"){echo "selected='selected'";}?>>PUBLIC ORGANIZATION</option>
    ...
    <div class="form-group" id ="category" name ="category" <?php if($group_details['Group']['group_type']=="F") { ?> style="display:none" <?php } ?>>
      ...
      <select class="form-control" name="category_id_B" id="category_id_B" <?php if($group_details['Group']['group_type']=="PO") { ?> style="display:none" <?php } ?> onChange="select_category(this.value);">
		...
		<select class="form-control" name="category_id_POG" id="category_id_POG" <?php if($group_details['Group']['group_type']=="B") { ?> style="display:none" <?php } ?> onChange="select_category(this.value);">
					 <option value="0">Select One</option>
					<?php 
						if(isset($all_categories_POG)) {
						   foreach($all_categories_POG as $data1) { ?>
			  <option value="<?php echo $data1['Category']['id'];?>"  <?php if($group_details['Group']['category_id'] == $data1['Category']['id']){ echo "selected=selected";}?>><?php echo $data1['Category']['title'];?></option>
			   <?php }}  ?>
		 </select>
       ...
       <div class="clearfix"></div>          
		...
		<button type="submit" class="btn signupbtn">Update Groop</button>
		</div>
		</div>
		</form>
	   <div class="clearfix"></div>
		...
		<div class="event-field">
			<div class="row">
				...
				<div class="col-sm-8">
					...
					<?php if($group_details['Group']['parent_id']!='0'){ ?>
					<span class="group-type">			
						<input type="radio" name="group_type" value="semi_private" id="group_type">
...
<!-- Create Request Sub Group Modal Start -->
<div class="modal fade" id="requestsubgroup" role="dialog">
    <div class="modal-dialog signup-model">    
		<div class="modal-content">
		  <!-- Modal content-->
		  ...
		  <div class="modal-header signup-header">
		<button type="button" class="close" data-dismiss="modal">×</button>
		<h4 class="modal-title">Create Sub Groop</h4>
		</div>
		   
		<div class="modal-body group-modal-body">
	   <form action="<?php echo $this->webroot?>group/add_sub_group" method="post" id="sub_group_form" name="sub_group_form" enctype="multipart/form-data">  
	   <input type="hidden" class="form-control" id="parent_id" name="parent_id" value="<?php echo $group_details['Group']['id'];?>">
	   <input type="hidden" class="form-control" id="group_user_type" name="group_user_type" value="<?php echo $group_member_type;?>">
	   
	  <div class="pop-createg">
	  
	  <div class="upload-img">
	  <div class="pop-createg-round">					 
			<img id="thumbnil100" class="no-profile-img" src="<?php echo $this->webroot?>images/no-group-img.jpg" />
	  </div>
	
	   
	   <span class="btn-bs-file browse-btn">Browse</span>
	   <input onChange="showMyImageSub(this)" name="upload_image" id="image455" data-badge="false" type="file"/>
	  <div class="clearfix"></div>
	  <p class="upload-img-info">please upload an image of min 160 px X 120 px</p>
		</div>
...
<!-- Create Request Sub Group Modal End -->
...
<main class="main-body">
  ...
  <div class="category-details-content">
    <div class="container"> 
	  ...
	  <?php if($show_delete == '1'){ ?>
        <div class="page-left-nav">
          <ul>           
             <li><a href="javascript:void(0)" onclick="delete_group(<?php echo $group_details['Group']['id'];?>);"> Delete <i class="fa fa-trash-o"></i></a></li>
          </ul>
        </div>
        <?php } ?>
		
      <?php if($group_member_type == 'owner' || $is_parent_grp_owner=='1'){ ?>
        <div class="page-left-nav">
          <ul>           
             <li><a href="javascript:void(0)" data-toggle="modal" data-target="#editgroup"> Edit <i class="fa fa-pencil-square-o"></i></a></li>
            <li><a class="userbox view-user" href="<?php echo $this->webroot?>group/push_message/<?php echo $group_details['Group']['id'];?>">Push your message <i class="fa fa-bell"></i></a></li>
          </ul>
        </div>
        <?php } 
		else if($group_member_type == 'member' && $is_parent_grp_owner=='0'){ ?>
		<div class="page-left-nav">
          <ul>           
            <li><a class="userbox view-user" href="<?php echo $this->webroot?>group/push_message_to_owners/<?php echo $group_details['Group']['id'];?>">Push your message <i class="fa fa-bell"></i></a></li>
          </ul>
        </div>
		<?php
		}
	  ...
	  <?php 
	  	if($this->Session->read('userData') && $group_details['Group']['parent_id']=='0'){
			if($group_member_type == 'owner'){ 
	  ?>
			<div class="page-right-nav">
			  <ul>           
				 <li><a href="javascript:void(0)" data-toggle="modal" data-target="#requestsubgroup"> Create Sub Groop <i class="fa create-icon"></i></a></li>
			  </ul>
			</div>
			<?php } 
			else{
			...
			<div class="page-right-nav">
			  <ul>           
				 <li><a href="javascript:void(0)" data-toggle="modal" data-target="#requestsubgroup">Request Sub Groop <i class="fa create-icon"></i></a></li>
			  </ul>
			</div>
		<?php
			}
      ...
      <?php if($group_member_type == 'owner'){ ?>
        <div class="page-right-nav">
          <ul>
            <li><a  class="userbox view-user" href="<?php echo $this->webroot?>group/group_member_list/<?php echo $group_details['Group']['id'];?>" > View users <i class="fa fa-user"></i></a></li>

            <li><a class="userbox view-user" href="<?php echo $this->webroot?>group/invite_list/<?php echo $group_details['Group']['id'];?>">Send Invitation <i class="fa fa-user-plus"></i></a></li>
          </ul>
        </div>
      <?php } else if($group_member_type == 'member'){ 
	  		if($is_parent_grp_owner=='0'){
	  ?>
	  <div class="page-left-nav">
			  <ul>
        ...
        <div class="page-right-nav">
          <ul>
			...
			<?php 
				if($is_parent_grp_owner=='0'){
			?>
           <li><a href="#" data-toggle="modal" data-target="#leave_group"> Leave Groop  <i class="fa create-icon"></i></a></li>
		   <?php } ?>
            <li><a class="userbox view-user" href="<?php echo $this->webroot?>group/recommend_list/<?php echo $group_details['Group']['id'];?>">Send Recommendation <i class="fa fa-user-plus"></i></a></li>
          </ul>
        </div>
      <?php } else {
	   ...
	   ?>
          ...
          <?php  $GroupIsJoin = $this->requestAction('group/group_is_join/'.$group_details['Group']['id'].'/');
                 if($GroupIsJoin > 'O') { ?>
                <li><a href="javascript:void(0)" class="group-btn invite">Request Sent <i class="fa fa-user-plus"></i></a></li>
                <?php }else{ ?>
                <li><a href="javascript:void(0)" data-toggle="modal" data-target="#joinnow" data-id="<?php echo $group_details['Group']['id'];?>" >Join Now <i class="fa fa-user-plus"></i></a></li>
                <?php } ?>
     ...
     <?php }}} ?>
      ...
      <div class="group-details-info">
        <div class="group-details-img">
			...
			<?php 
				...
				if($group_details['Group']['parent_id']=='0'){
					...
					if($group_details['Group']['icon']!=''){ ?>
					<img id="thumbnil" src="<?php echo $this->webroot;?>sub_group_images/<?php echo $group_details['Group']['icon']; ?>" alt="<?php echo ucfirst(stripslashes($group_details['Group']['group_title'])); ?>">
        ...
        <div class="group-details-content">
          <h4>
		  	<?php  echo ucfirst(stripslashes($group_details['Group']['group_title']));  ?>
		  </h4>
	...
	<div class="col-md-3">
        ...
        $('#wrapper .version strong').text('v' + $.fn.pignoseCalendar.version);
				...
				function onNext(info, context){
					...
					jQuery.ajax({
					  type: "GET",
					  url: WEBROOT+"group/fetch_event_dates",
					  data: {group_id:group_id, other_month:next_month_date},
					  success: function(response){
					  		if(response!=''){
				...
				function onPrev(info, context){
					...
					jQuery.ajax({
					  type: "GET",
					  url: WEBROOT+"group/fetch_event_dates",
					  data: {group_id:group_id, other_month:prev_month_date},
					  success: function(response){
					  		if(response!=''){
			...
			<div class="upcoming-events-list">
              ...
              <div class="event-location">               
			   ...
			   <div class="event-des">
                ...
                <h4 class="headstyle"><?php echo $events['event_name']; ?></h4>
				<div class="clearfix"></div>
...
<?php if(($group_details['Group']['group_type']=='B' || $group_details['Group']['group_type']=='PO') && $group_user_type=='O'){?>
  ...
  <?php foreach($ArGroupMessage as $key=>$GroupMessage){?>
		...
		<?php } ?>
		  ...
		  <div class="message-reply-row">
            ...
            <a href="javascript:void(0)" class="Reply_Assign_link edit-reply" data-id="<?php echo $MessageReply['id'];?>"  data-reply="<?php echo $MessageReply['reply'];?>" data-group_id='<?php echo $GroupMessage['GroupMessage']['group_id'];?>' data-toggle="modal" data-target="#editreplyModal">
  						<i class="fa fa-pencil"></i>
  					</a>
