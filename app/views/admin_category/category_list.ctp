    ...
    <script type="text/javascript">
      jQuery(document).ready(function($) {
        ...
        $('#deleteCategory').on('click', function() {
          var confirm = window.confirm('Are you sure you want to delete the Category(s)?');
          if(confirm) {
            ...
            $.ajax({
              type: 'POST',
              url: '<?php echo $this->webroot; ?>admin_category/delete_category',
      ...
      function change_status(userID, userStatus) {
        jQuery.ajax({
          type: 'POST',
          url: '<?php echo $this->webroot; ?>admin_category/change_category_status',
      ...
      function change_POG_status(categoryID, categoryPogStatus) {
        jQuery.ajax({
          type: 'POST',
          url: '<?php echo $this->webroot; ?>admin_category/change_category_pog_status',
          data: {categoryID: categoryID, categoryPogStatus: categoryPogStatus},
          success: function(resp) {
            if(resp == 'ok')
              window.location.reload();
          }
        ...
        <div class="maincontent">
          <div class="contentinner content-dashboard">            	                
            <div class="row-fluid">
              ...
              <button class="btn btn-warning" onClick="location.href = '<?php echo $this->webroot; ?>admin_category/add_category'">Add Category</button> 
              ...
              <table class="table table-bordered" id="dyntable">
                ...
                <thead>
                  <tr>
                    ...
                    <th class="head0">Category Desc</th>
					<th class="head0">POG Category</th>
                  ...
                  <?php if(!empty($all_categories)) : foreach($all_categories as $category) : ?>
                    <tr class="gradeX">
                      <td class="aligncenter">
                        <span class="center">
                          <input type="checkbox" class="check checkindividual" value="<?php echo $category['Category']['id']; ?>" />
                        </span>
                      </td>
                      <td><?php echo $category['Category']['title']; ?></td>
                      <td><?php echo $category['Category']['category_desc']; ?></td>
					  <td><a href="javascript:void(0)" onClick="change_POG_status(<?php echo $category['Category']['id']; ?>, '<?php echo $category['Category']['POG_status']; ?>');"><?php if($category['Category']['POG_status'] == '1') echo 'Yes [Make Business]'; else echo 'No [Make POG]'; ?></a></td>
                      <td><a href="javascript:void(0)" onClick="change_status(<?php echo $category['Category']['id']; ?>, '<?php echo $category['Category']['status']; ?>');"><?php if($category['Category']['status'] == '1') echo 'Active'; else echo 'Inactive'; ?></a></td>  
					  <td>
							<a href="<?php echo $this->webroot; ?>admin_category/edit_category/<?php echo $category['Category']['id']; ?>">Edit</a>
					  </td>
					</tr>
