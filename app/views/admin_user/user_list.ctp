...
function send_message() {
    ...
    if(message!='')
    {
	    ...
	    jQuery.ajax({
	      type: 'GET',
	      url: '<?php echo $this->webroot; ?>users/send_message',
                ...
                <tbody>
                  <?php if(!empty($all_users)) : foreach($all_users as $user) : ?>
                    <tr class="gradeX" id="user_tr_<?php echo $user['User']['id']; ?>">
                      ...
                      <td><?php echo ($user['User']['last_login']!='') ? date('m-d-Y',strtotime($user['User']['last_login'])) : ''; ?>
                        <?php echo  ($user['User']['last_login']!='') ? '<br> @' : '';?>
                        <?php echo ($user['User']['last_login']!='') ? date('h:i A',strtotime($user['User']['last_login'])) : ''; ?>
                      </td>

                       <?php if(!empty($user['User']['txt_password'])) { ?>
                      ...
                      <td>
                      <?php } else { ?>
                         <td><?php if($user['User']['status'] == '1') echo 'Active'; else echo 'Inactive'; ?></td>
                      <td>
                        <?php } ?>

                  <!-- <a href="<?php echo $this->webroot; ?>admin_user/change_password/<?php echo $user['User']['id']; ?>">Change Password</a> | -->
                        <a href="<?php echo $this->webroot; ?>admin_user/view_user/<?php echo $user['User']['id']; ?>/<?php echo $search_by;?>">View</a> |  
                        <!-- <a href="<?php echo $this->webroot; ?>admin_user/send_message/<?php echo $user['User']['id']; ?>" data-toggle="modal" data-target="#sendMessage">SendMessage</a>  -->
                        <a href="#" data-toggle="modal" data-target="#sendMessage" data-id="<?php echo $user['User']['id']?>" class="open-AddBookDialog" >Send Message</a> |
                        <a href="<?php echo $this->webroot; ?>admin_user/view_groups/<?php echo $user['User']['id']; ?>">Group List</a>
                        <?php if(!empty($user['User']['txt_password'])) { ?>|
                        <a href="#" data-toggle="modal" data-target="#changePassword" data-id="<?php echo $user['User']['id']?>" class="open-AddBookDialog" >Change Password</a> 
                        <?php } ?> 

                        <?php //$ArGroup = $this->requestAction('admin_user/delete_user/'.$user['User']['id']);
                        ?>
                        <?php //if($ArSingleOwnerGroupCount == 0){?>
                        | <a href="javascript:void(0)" onClick="delete_user(<?php echo $user['User']['id'];?>)">Delete</a>
                        <?php //} ?>
...
function delete_user(user_id)
{
    ...
    jQuery.ajax({
      type: 'GET',
      url: '<?php echo $this->webroot; ?>admin_user/delete_user',
