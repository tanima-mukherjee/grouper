                          ...
                          <?php } ?>
												...
												if(($list['Notification']['sender_type']== 'GO')&& ($list['Notification']['receiver_type']== 'NGM')) { ?>
												  <div class="message-div" >
													...
													<span><i><?php echo ucfirst($sender_details['User']['fname'] .' '. $sender_details['User']['lname']) ;?></i></span> sent you a request to approve his created Sub Groop: <span><?php  echo ucfirst(stripslashes($group_details['Group']['group_title']));?></span> .    
...
<script>
	...
	$('.list-height').on('scroll', function() {
        if($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight) {
           // alert('end reached');
		   var start_val= parseInt($('#start').val())+parseInt($('#limit').val());
		   var limit_val= $('#limit').val();
		   var total_notification= parseInt($('#total_notification').val());
		   if(start_val<total_notification){
			   jQuery.ajax({
				type: 'GET',
				url: '<?php echo $this->webroot; ?>home/notification_showmore',
				data: {start: start_val, limit:limit_val},
				success: function(resp){
					//alert('Alok');
					//alert(resp);
					$(".list-height ul").append(resp);
					$('#start').val(start_val);
					$('#limit').val(limit_val);
				}
			  });
