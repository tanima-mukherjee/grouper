...
function lookup_friends(inputString, group_id){
	var str_selected_users=$('#str_selected_users1').val();
	if(inputString.length < 3) {
		...
		$.ajax({
           type: "GET",
           url: WEBROOT+"group/get_recommended_list_friends",
...
function fill_friends(fname, lname, userid){
	...
	var selected_users= jQuery('#str_selected_users1').val();
	if(selected_users == ''){
	...
	jQuery('#str_selected_users1').val(selected_users);
}

function remove_friend(userid){
	$('#friend_'+userid).remove();
	
	var current_users= jQuery('#str_selected_users1').val();
	...
	if(current_users.charAt(0)==','){
		...
		jQuery('#str_selected_users1').val(current_users);
	}
...
function submit_friend_recommendation(){
	var group_id= $('#group_id1').val();
	var str_users= $('#str_selected_users1').val();
	var mode= $('#mode').val();
	var sender_type= $('#sender_type1').val();
	var group_type= $('#group_type1').val();
	
	if(str_users!=''){
		$.ajax({
			   type: "GET",
			   url: WEBROOT+"group/submit_recommendation",
			   data: {mode:mode, group_id:group_id, str_users:str_users, sender_type:sender_type, group_type:group_type},
			   success: function(msg){
			   if(msg!=0){ 
			   		$('#str_selected_users1').val(''); 
					$("#selected_friends ul > li").remove();
					$('#success_msg').show();
					$('#success_msg').html('Recommendation sent successfully !!!');	 
...
function lookup_users(inputString, group_id){
	...
	if(inputString.length < 3) {
		...
		$.ajax({
           type: "GET",
           url: WEBROOT+"group/get_recommended_list_users",
           data: {search_text:inputString, group_id:group_id, str_selected_users:str_selected_users},
           success: function(msg){
           if(msg!=0){ 
...
function submit_user_recommendation(){
	var group_id= $('#group_id2').val();
	var str_users= $('#str_selected_users2').val();
	var mode= $('#mode2').val();
	var sender_type= $('#sender_type2').val();
...
function submit_email_recommendation(){
	var group_id= $('#group_id3').val();
	var str_users= $('#str_selected_users3').val();
	var mode= $('#mode3').val();
	var sender_type= $('#sender_type3').val();
	var group_type= $('#group_type3').val();
	...
	<div class="user-dialog">
	 ...
	 <div id="success_msg" style="display:none; background:#ccc; padding:10px 12px; font-weight:bold; font-size:16px">Sourov</div>
      ...
      <div class="col-sm-12 fields-margin" id="section1">
		...
		<input type="hidden" name="mode" id="mode" value="recommend_friends">
		<input type="hidden" name="str_selected_users1" id="str_selected_users1" value="">
		<input type="hidden" name="group_id1" id="group_id1" value="<?php echo $group_id; ?>">
		<input type="hidden" name="sender_type1" id="sender_type1" value="GM">
		<input type="hidden" name="group_type1" id="group_type1" value="<?php echo $group_detail['Group']['group_type']; ?>">
		...
		<div class="text-center"><input type="button" value="submit" class="popup_submit" onClick="submit_friend_recommendation();"></div>
	    <div class="clearfix"></div>
		</form>
	  </div>
	  ...
	  <div class="col-sm-12 fields-margin" id="section2" style="display:none">
		...
		<input type="hidden" name="mode2" id="mode2" value="recommend_users">
		<input type="hidden" name="str_selected_users2" id="str_selected_users2" value="">
		<input type="hidden" name="group_id2" id="group_id2" value="<?php echo $group_id; ?>">
		<input type="hidden" name="sender_type2" id="sender_type2" value="GM">
		<input type="hidden" name="group_type2" id="group_type2" value="<?php echo $group_detail['Group']['group_type']; ?>">
		...
		<div class="clearfix"></div>
		<div class="text-center"><input type="button" value="submit" class="popup_submit" onClick="submit_user_recommendation();"></div>
	   <div class="clearfix"></div>
	   </form>
	  </div>
	  
	  <div class="col-sm-12 fields-margin" id="section3" style="display:none">
		<label class="profilelabel">Recommend Via Email *</label>
	
		<form name="submit_emails" id="submit_emails" method="post" action="">
		<input type="hidden" name="mode3" id="mode3" value="recommend_emails">
		<input type="hidden" name="str_selected_users3" id="str_selected_users3" value="">
		<input type="hidden" name="group_id3" id="group_id3" value="<?php echo $group_id; ?>">
		<input type="hidden" name="sender_type3" id="sender_type3" value="GM">
		<input type="hidden" name="group_type3" id="group_type3" value="<?php echo $group_detail['Group']['group_type']; ?>">
		<div class="profilefield">
		...
		<div class="clearfix"></div>
		<div class="text-center"><input type="button" value="submit" class="popup_submit" onClick="submit_email_recommendation();"></div>
	   <div class="clearfix"></div>
	   </form>
	  </div>
