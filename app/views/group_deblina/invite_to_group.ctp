...
<!-- <script>
  ...
  function more_user_found(group_id){
  //alert(group_id);
    if(group_id != ''){ 
       jQuery.ajax({

          type: "GET",
          url: WEBROOT+"group/group_member_list",
          data: {group_id:group_id},
          success: function(response){
          //alert(response);
           jQuery("#usermodal").html(response);
          }
      });
    ...
    <div class="container">
	  ...
	  <div class="page-right-nav">
		<ul>	
		  ...
		  <li>
			<a href="javascript:void(0)" data-toggle="modal" data-target="#creategroup" class="create-link fright">Create Group <i class="fa create-icon"></i></a>
		  </li>
	  </ul>
	  </div>
	  </div>
            ...
            <div class="groupbox">
             ...
             <img src="<?php echo $this->webroot.'group_images/web/'.$list['Group']['icon'];?>" alt=""/> <?php } else { ?><img src="<?php echo $this->webroot?>images/no_profile_img.jpg" alt="" /> <?php } ?>
              ...
              <?php }  else if ($GroupType['GroupUser']['user_type'] == 'M') {  ?> 
							<a href="javascript:void(0)" class="group-btn invite">Recommend </a>
              <?php } else {
