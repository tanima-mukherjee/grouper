...
<div class="calendar" id="cal_po"></div>
...
$('#wrapper .version strong').text('v' + $.fn.pignoseCalendar.version);
	...
	function onSelectHandler(date, context) {
		...
		jQuery.ajax({
		  type: "GET",
		  url: WEBROOT+"event/po_group_event_list",
	...
	function onNext(info, context){
		...
		jQuery.ajax({
		  type: "GET",
		  url: WEBROOT+"event/fetch_event_dates_po_grp",
...
<?php
	if(count($arr_event_dates_po) > 0){
	for($k=0; $k<count($arr_event_dates_po); $k++){
	
	if($arr_event_dates_po[$k]==$today){
?>				
		jQuery("#cal<?php echo $arr_event_dates_po[$k] ?>").css("background-color", "#FF0000");
<?php
	}
else{
?>
		jQuery("#cal<?php echo $arr_event_dates_po[$k] ?>").css("background-color", "#7B68EE");
		jQuery("#cal<?php echo $arr_event_dates_po[$k] ?>").css("color", "#ffffff");
<?php
	}
