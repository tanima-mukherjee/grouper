...
class HomeController extends AppController {
	...
	$email = $this->params['form']['username'];
		...
		if(!empty($detail)){
		  ...
		  if ($detail['User']['status'] == 1){
		  	...
		  	$this->data['User']['id'] = $detail['User']['id'];
	...
	function my_group_po(){
		...
		if(empty($all_my_groups)){
			...
			if($count_PO_groups>0){
				$limit = 12;
				$lastpage = ceil($count_PO_groups/$limit);
				$start=($_REQUEST['page'] - 1) * $limit;
				$page = $_REQUEST['page'];
				$prev = $page - 1;
				$next = $page + 1;
				...
				$all_PO_groups = $this->Group->find('all',array('conditions' =>$cond_PO_grps, 'offset' => $start, 'limit' => $limit, 'order' => 'Group.group_title ASC'));
...
//echo $created_date;
    ...
    function notification_list()
    {
		   ...
		   if( $is_user_exists >0)
		   {
			   ...
			   $notification_list = $this->Notification->find('all',array('conditions'=>$condition,'order'=>'Notification.id DESC', 'limit' => $ulimit));  
			   
			   
			   $this->set('notification_list',$notification_list);
			   $this->set('total_notification',$total_notification);
			   $this->set('limit',$limit);
			   $this->set('start',$start);
	...
	function notification_showmore()
    {
		   ...
		   if( $is_user_exists >0)
		   {
			   ...
			   $notification_list = $this->Notification->find('all',array('conditions'=>$condition,'order'=>'Notification.id DESC', 'limit' => $ulimit));  
			   $total_notification = count($notification_list);
    ...
    function unread_notifications(){
         ...
         $user_id = $this->Session->read('userData.User.id');
		 $condition = "Notification.receiver_id = '".$user_id."' AND Notification.status = '1' AND  ((Notification.is_reversed_notification = '0' AND Notification.is_receiver_accepted = '0' ) OR (Notification.is_reversed_notification = '1' AND Notification.is_receiver_accepted != '0') OR (Notification.is_receiver_accepted = '2' AND Notification.is_reversed_notification = '0' AND( (Notification.type = 'E')OR (Notification.type = 'P'))) OR (Notification.is_receiver_accepted = '2' AND Notification.is_reversed_notification = '0' AND Notification.type = 'G' AND Notification.message!='') OR (Notification.type = 'SG' AND Notification.is_receiver_accepted = '0' AND Notification.is_reversed_notification = '0') OR (Notification.type = 'SG' AND Notification.is_receiver_accepted = '2' AND Notification.is_reversed_notification = '1')) AND Notification.is_read='0'";
	...
	function fetch_event_dates_business(){
		...
		for($i=1;$i<=$days_this_month;$i++){
			 ...
			 if($current_day_this_month<= $last_day_this_month){
				 $con_count_events = "((Event.`event_date` = '".$current_day_this_month."') OR (Event.`event_start_date` <= '".$current_day_this_month."' AND Event.`event_end_date` >= '".$current_day_this_month."')) AND Event.`group_type` = 'B' AND Event.`type` = 'public'  AND Event.`status` = '1' AND `group_id` IN (". $allstatecitygroups .")";
				 $event_count = $this->Event->find('count',array('conditions' => $con_count_events));
    ...
    function community_event_list(){
          ...
          $event_details = $this->Event->query("SELECT * FROM ( SELECT `id`,`title`,`desc`,`group_type`,`type`,`event_timestamp` as `sort_time`,`group_id`,`status` ,`event_date`,`event_start_date` ,`event_end_date`,`deal_amount`,`location`,`latitude`,`longitude`,`is_multiple_date` FROM `events` WHERE (`group_type` = 'F' OR `group_type` = 'PO') AND `type` = 'public' AND `status` = '1' AND `event_date` = '".$selectdate."' AND `group_id` IN (". $allstatecitygroups .") UNION SELECT `id`,`title`,`desc`,`group_type`,`type`,`event_start_timestamp` as `sort_time`,`group_id`,`status` ,`event_date`,`event_start_date` ,`event_end_date`,`deal_amount`,`location`,`latitude`,`longitude`,`is_multiple_date` FROM `events` WHERE (`group_type` = 'F' OR `group_type` = 'PO') AND `type` = 'public' AND `status` = '1' AND `event_start_date` <= '".$selectdate."' AND `event_end_date` >= '".$selectdate."' AND `group_id` IN (". $allstatecitygroups .")) AS `Event`ORDER BY `Event`.`sort_time`ASC");
	...
	function fetch_event_dates(){
		...
		for($i=1;$i<=$days_this_month;$i++){
			 ...
			 if($current_day_this_month<= $last_day_this_month){
				 $con_count_events = "((Event.`event_date` = '".$current_day_this_month."') OR (Event.`event_start_date` <= '".$current_day_this_month."' AND Event.`event_end_date` >= '".$current_day_this_month."')) AND (Event.`group_type` = 'F' OR Event.`group_type` = 'PO') AND Event.`type` = 'public'  AND Event.`status` = '1' AND `group_id` IN (". $allstatecitygroups .")";
				 $event_count = $this->Event->find('count',array('conditions' => $con_count_events));
			...
			//$group_type = $_REQUEST['group_type'];
			 ...
			 if(isset($mode) && $mode =='invite_users'){
				...
				for($i=0; $i<count($arr_users); $i++){
						...
						$con_is_req_sent = "Notification.status = '1' AND  Notification.type = 'F' AND Notification.sender_id = '".$user_id."' AND Notification.receiver_id = '".$arr_users[$i]."' AND Notification.is_receiver_accepted = '0' AND Notification.is_reversed_notification = '0' ";
	...
	function category_list(){
      ...
      $this->layout = "";
        ...
        $condition4 = "Category.status= '1' AND Category.POG_status= '0'";
	...
	function category_list_POG(){

  	  // $this->_checkSessionUser();

      $this->layout = "";
     
        $condition4 = "Category.status= '1' AND Category.POG_status= '1'";
        $category_details_POG = $this->Category->find('all',array('conditions'=>$condition4));
            
    	//pr($category_details);exit();
       return $category_details_POG;
