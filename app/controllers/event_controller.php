...
class EventController extends AppController {
            ...
            $this->_checkSessionUser();
              ...
              $user_id = $this->Session->read('userData.User.id');
                 ...
                 //  pr($group_detail['Group']['category_id']);
                   ...
                   else if($group_detail['Group']['group_type']=='F' || $group_detail['Group']['group_type']=='PO')
                   {
                    $this->data['Event']['deal_amount'] = '0.00';
                  ...
                  {
				  	strtotime($this->params['form']['event_date']); 
    ...
    function notification_email($notification_id,$eve_name){
     ...
     function event_list()
     {
		...
		if($str_subgroups!=''){
			$event_details = $this->Event->query("SELECT * FROM ( SELECT `id`,`title`,`desc`,`group_type`,`type`,`event_timestamp` as `sort_time`,`group_id`,`status` ,`event_date`,`event_start_date` ,`event_end_date`,`deal_amount`,`location`,`latitude`,`longitude`,`is_multiple_date` FROM `events` WHERE (`group_id` = '".$group_id."' OR (`group_id` IN (".$str_subgroups.") AND (`type` = 'public' OR `type` = 'semi_private'))) AND `status` = '1' AND `event_date` = '".$selectdate."' UNION SELECT `id`,`title`,`desc`,`group_type`,`type`,`event_start_timestamp` as `sort_time`,`group_id`,`status` ,`event_date`,`event_start_date` ,`event_end_date`,`deal_amount`,`location`,`latitude`,`longitude`,`is_multiple_date` FROM `events` WHERE (`group_id` = '".$group_id."' OR (`group_id` IN (".$str_subgroups.") AND (`type` = 'public' OR `type` = 'semi_private'))) AND `status` = '1' AND`event_start_date` <= '".$selectdate."' AND `event_end_date` >= '".$selectdate."') AS `Event`ORDER BY `Event`.`sort_time`ASC");
		}
		else{
    ...
    function my_calender(){
	   ...
	   // Public Prganization my calender end //
		...
		/***********************      Fetch the dates with Events starts       **********************/
			...
			for($i=1;$i<=$days_this_month;$i++){
				 ...
				 if($current_day_this_month<= $last_day_this_month){
					 $con_count_events = "((Event.`event_date` = '".$current_day_this_month."') OR (Event.`event_start_date` <= '".$current_day_this_month."' AND Event.`event_end_date` >= '".$current_day_this_month."')) AND Event.`group_type` = 'F' AND Event.`status` = '1' AND Event.`group_id` IN (". $allgroups .")";
	...
	function my_calendar_free(){
	  ...
	  // free my calender end //
	   ...
	   for($i=1;$i<=$days_this_month;$i++){
			 ...
			 if($current_day_this_month<= $last_day_this_month){
				 $con_count_events = "((Event.`event_date` = '".$current_day_this_month."') OR (Event.`event_start_date` <= '".$current_day_this_month."' AND Event.`event_end_date` >= '".$current_day_this_month."')) AND Event.`group_type` = 'F' AND Event.`status` = '1' AND `group_id` IN (". $allgroups .")";
				 $event_count = $this->Event->find('count',array('conditions' => $con_count_events));
    ...
    function fetch_event_dates_private_grp(){
	...
	function my_calendar_business(){
	   ...
	   $days_this_month= date('t');
	   $current_day_this_month= $first_day_this_month;
	   $arr_event_dates_business = array();
	   $current_day_this_month= $first_day_this_month;
	   for($i=1;$i<=$days_this_month;$i++){
			
			 if($current_day_this_month<= $last_day_this_month){
				 $con_count_events = "((Event.`event_date` = '".$current_day_this_month."') OR (Event.`event_start_date` <= '".$current_day_this_month."' AND Event.`event_end_date` >= '".$current_day_this_month."')) AND Event.`group_type` = 'B' AND Event.`status` = '1' AND Event.`group_id` IN (". $allgroups .")";
				 $event_count = $this->Event->find('count',array('conditions' => $con_count_events));
				 
				 if($event_count>0){
					array_push($arr_event_dates_business, $current_day_this_month);
				 }
		...
		$this->set('arr_event_dates_business',$arr_event_dates_business);
	}
	

	function fetch_event_dates_business_grp(){
		...
		for($i=1;$i<=$days_this_month;$i++){
			 ...
			 if($current_day_this_month<= $last_day_this_month){
				 $con_count_events = "((Event.`event_date` = '".$current_day_this_month."') OR (Event.`event_start_date` <= '".$current_day_this_month."' AND Event.`event_end_date` >= '".$current_day_this_month."')) AND Event.`group_type` = 'B' AND Event.`status` = '1' AND `group_id` IN (". $allgroups .")";
				 $event_count = $this->Event->find('count',array('conditions' => $con_count_events));
	...
	function my_calendar_po(){
		...
		// Public Prganization my calender start //
        
      $event_po_group_details = $this->Event->query("SELECT * FROM ( SELECT `id`,`title`,`desc`,`group_type`,`type`,`event_timestamp` as `sort_time`,`group_id`,`status` ,`event_date`,`event_start_date` ,`event_end_date`,`deal_amount`,`location`,`latitude`,`longitude`,`is_multiple_date` FROM `events` WHERE `group_type` = 'PO' AND `status` = '1' AND `event_date` = '".$date."' AND `group_id` IN (". $allgroups .") UNION SELECT `id`,`title`,`desc`,`group_type`,`type`,`event_start_timestamp` as `sort_time`,`group_id`,`status` ,`event_date`,`event_start_date` ,`event_end_date`,`deal_amount`,`location`,`latitude`,`longitude`,`is_multiple_date` FROM `events` WHERE `group_type` = 'PO' AND `status` = '1' AND `group_id` IN (". $allgroups .") AND`event_start_date` <= '".$date."' AND `event_end_date` >= '".$date."') AS `Event`ORDER BY `Event`.`sort_time`ASC");
	  
        $event_PO_group_list = array(); 
		
		if(!empty($event_po_group_details)){
			foreach($event_po_group_details as $events){
				  ...
				  array_push($event_PO_group_list,$list);
			}
		}

		$this->set('event_PO_group_list',$event_PO_group_list);
		//$this->set('selectdate',$selectdate);  
	  ...
	  // PO my calender end //
	   ...
	   $days_this_month= date('t');
	   $current_day_this_month= $first_day_this_month;
	   $arr_event_dates_po = array();
		for($i=1;$i<=$days_this_month;$i++){
			
			 if($current_day_this_month<= $last_day_this_month){
				 $con_count_events = "((Event.`event_date` = '".$current_day_this_month."') OR (Event.`event_start_date` <= '".$current_day_this_month."' AND Event.`event_end_date` >= '".$current_day_this_month."')) AND Event.`group_type` = 'PO' AND Event.`status` = '1' AND Event.`group_id` IN (". $allgroups .")";
				 $event_count = $this->Event->find('count',array('conditions' => $con_count_events));
				 
				 if($event_count>0){
					array_push($arr_event_dates_po, $current_day_this_month);
				 }
		...
		$this->set('arr_event_dates_po',$arr_event_dates_po);
	}
	
	
	function fetch_event_dates_po_grp(){
		...
		for($i=1;$i<=$days_this_month;$i++){
			 ...
			 if($current_day_this_month<= $last_day_this_month){
				 $con_count_events = "((Event.`event_date` = '".$current_day_this_month."') OR (Event.`event_start_date` <= '".$current_day_this_month."' AND Event.`event_end_date` >= '".$current_day_this_month."')) AND Event.`group_type` = 'PO' AND Event.`status` = '1' AND `group_id` IN (". $allgroups .")";
				 $event_count = $this->Event->find('count',array('conditions' => $con_count_events));
	...
	function po_group_event_list(){
       ...
       $event_po_group_details = $this->Event->query("SELECT * FROM ( SELECT `id`,`title`,`desc`,`group_type`,`type`,`event_timestamp` as `sort_time`,`group_id`,`status` ,`event_date`,`event_start_date` ,`event_end_date`,`deal_amount`,`location`,`latitude`,`longitude`,`is_multiple_date` FROM `events` WHERE `group_type` = 'PO' AND `status` = '1' AND `event_date` = '".$selectdate."' AND `group_id` IN (". $allgroups .") UNION SELECT `id`,`title`,`desc`,`group_type`,`type`,`event_start_timestamp` as `sort_time`,`group_id`,`status` ,`event_date`,`event_start_date` ,`event_end_date`,`deal_amount`,`location`,`latitude`,`longitude`,`is_multiple_date` FROM `events` WHERE `group_type` = 'PO' AND `status` = '1' AND `group_id` IN (". $allgroups .") AND`event_start_date` <= '".$selectdate."' AND `event_end_date` >= '".$selectdate."') AS `Event`ORDER BY `Event`.`sort_time`ASC");
