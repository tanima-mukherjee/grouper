...
class GroupController extends AppController {
    ...
    var $name = 'Group';
    var $uses = array('Category','Group','SubscriptionPlan','User','GroupUser','City','State','GroupImage','GroupDoc','Video','Notification','Friendlist');
  ...
  function join_request()
  {
	 ...
	 $this->layout = "";
		  ...
		  $this->set('pageTitle', 'Group Type');
			...
			$condition_check_request_sent_already = "Notification.group_id = '".$this->params['form']['group_of_id']."' AND Notification.sender_id = '".$user_id."' AND Notification.sender_type = 'NGM' AND Notification.request_mode = '".$this->params['form']['group_type']."' AND Notification.receiver_id IN (". $group_detail['Group']['group_owners'].") AND Notification.receiver_type = 'GO' AND Notification.group_type = '".$group_detail['Group']['group_type']."' AND Notification.is_receiver_accepted = '0' AND Notification.is_reversed_notification = '0'";	//Request to join already sent or not starts 
                ...
                if($this->Notification->save($this->data))
                    {
                      ...
                      { 
					    ...
					    $last_ins_id = $this->GroupUser->getLastInsertId(); 
                      
                        $user_id= $noti_detail['Notification']['receiver_id'];
                        $condition10 = "User.id = '".$user_id."'";
                        $user_details = $this->User->find('first',array('conditions'=>$condition10)); 

                        if(!empty($user_details['User']['groups']))
                        {

                        $this->data['User']['id'] = $user_details['User']['id'];
                        $this->data['User']['groups'] = $user_details['User']['groups'].",".$grp_user_detail['GroupUser']['group_id'];
                        ...
                        {
                           $this->data['User']['id'] = $noti_detail['Notification']['receiver_id'];
	  ...
	  function get_suggestion_list_friends(){
		...
		if(count($search_friend_list) > 0)
        {
			foreach($search_friend_list as $key => $val){
				...
				if($count_is_member == 0)			//friend does not belong to selected group
				{
					//unset($search_friend_list[$key]);
		...
		$this->set('arr_not_group_friends',$arr_not_group_friends);		
	  }
      ...
      function invite_to_group()
          {
            ...
            $email_list = array("abc@gmail.com","bcd@gmail.com","cde@gmail.com");

            foreach($email_list as $emails)
            
            {
                   
                    $this->data['User']['email'] = $emails;
