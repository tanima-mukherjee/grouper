<?php 
class TestAndroidController  extends AppController 
{
    var $name = 'TestAndroid';
    var $helpers = array('Html', 'Form','Javascript','Js','Session');
    var $components = array('RequestHandler', 'Session','Cookie','Email');  
    var $uses = array("Chat","Admin","User","Category","Group","Event","GroupImage","City","State","GroupUser","SiteSetting","EmailTemplate","GroupDoc","Video","Notification","Friendlist","Testimonial");
	...
	function login(){
		...
		$password = '123456';
		$device_token= 'dKH2FWX6DtQ:APA91bHchG6PmkkWWic1S_BcBwiYPMt1dFjhlbilgQ1wXv0symPqwsNqHkFLBEClgb88YMa92weHi-JDefahsIESD3Ib5Wg2TKEeprwAjQYxsX9Nej60ghr8czb_qnisnSY3WmBZtusD';
		$device_type= 'android';*/
		...
		$condition = "User.email = '".$email."' AND User.txt_password = '".$password."' AND User.password = '".md5($password)."' AND User.status = '1'";
		$detail = $this->User->find('first',array('conditions'=>$condition));
				
		//pr($detail_count);exit();
		if(!empty($detail)){
		
			$this->User->query("UPDATE `users` SET `device_token` ='".$device_token."'  WHERE `id` = '".$detail['User']['id']."'");
	...
	function category_details(){
        ...
        $city_id = '9531';*/
         ...
         $condition = "Group.category_id = '".$category_id."' AND Group.status = '1' AND Group.city_id = '".$city_id."' AND Group.state_id = '".$state_id."'";
	...
	function edit_group(){
	   ...
	   /* $user_id = '2';
		$group_id = '10'; 
		$group_name = 'group test 9'; 
		$description = 'group test desc 9'; 
		$purpose = 'group test desc 9'; */
		
		
		$condition_group_detail = "GroupUser.group_id = '".$group_id."' AND GroupUser.user_id = '".$user_id."' AND GroupUser.user_type = 'O'";
	...
	function push_message(){
            ...
            /*$sender_id = '4';            
            $group_id = '45';
            $message = "test app push msg";
            $member_list = array("6","137");*/
	...
	function make_owner(){
		...
		if(!empty($arr_receiver_status)){
			...
			if($arr_receiver_status['GroupUser']['user_type']=='M'){
				$this->GroupUser->query("UPDATE `group_users` SET `user_type` ='O' WHERE `group_id` = '".$group_id."' AND `user_id` = '".$receiver_id."'");
	...
	function create_event(){
		...
		$end_datetime    = $obj->{'end_datetime'};
         ...
         /*   $title    = 'night party test';
            $group_id    = '44';
            $type    = 'private';
            $datetime    = '2017-06-30 20:34:00';
            $desc    = 'Well done folks';
	...
	function event_list(){
       ...
       $event_details = $this->Event->query("SELECT * FROM ( SELECT `id`,`title`,`desc`,`group_type`,`type`,`event_timestamp` as `sort_time`,`group_id`,`status` ,`event_date`,`event_start_date` ,`event_end_date`,`deal_amount`,`location`,`latitude`,`longitude`,`is_multiple_date` FROM `events` WHERE `group_id` = '".$group_id."' AND `status` = '1' AND `event_date` = '".$date."' UNION SELECT `id`,`title`,`desc`,`group_type`,`type`,`event_start_timestamp` as `sort_time`,`group_id`,`status` ,`event_date`,`event_start_date` ,`event_end_date`,`deal_amount`,`location`,`latitude`,`longitude`,`is_multiple_date` FROM `events` WHERE `group_id` = '".$group_id."' AND `status` = '1' AND`event_start_date` <= '".$date."' AND `event_end_date` >= '".$date."') AS `Event`ORDER BY `Event`.`sort_time`ASC");
...
/*
	...
	function group_video_upload(){
        ...
        if(!empty($_FILES["video_file"]))
        {
                ...
                $video_fileType = $_FILES["video_file"]['type'];
                $video_file_arr = explode('.',$video_file);
                $video_file_arr_count = sizeof($video_file_arr);
                $upload_video = "video_file_".time().".".$video_file;
...
*/
				...
				if(move_uploaded_file($_FILES["video_file"]['tmp_name'],$upload_target_original))
				{
						// $response['is_error'] = 0;
						 $is_upload_video = 1; 
				}
				...
				{
					$response['is_error'] = 1;
					$response['err_msg'] = 'Video upload failed'; 
				}
		...
		if(move_uploaded_file($_FILES["upload_image"]['tmp_name'], $upload_image_target_original)) {
		...
		// $group_id = '6';
		  ...
		  if(!empty($video_list))
		  {
			foreach($video_list as $key => $val){   
				 ...
				 $response['group_video'][$key]['image'] = $base_url.'group_videos/images/'.$val['Video']['v_image'];
				 $response['group_video'][$key]['video'] = $base_url.'group_videos/'.$val['Video']['video'];
				 $response['group_video'][$key]['video_name'] = $val['Video']['v_name'];
	...
	function delete_group_video(){
		...
		if(!empty($GrpVideodetail)){
				...
				$folder = 'group_videos/';
				$folder_image = 'group_videos/images/';
				
				$this->removeFile($GrpVideodetail['Video']['video'],$folder);
				$this->removeFile($GrpVideodetail['Video']['v_image'],$folder_image);
        ...
        if( $is_user >0){
            ...
            if(count($notification_list) > 0){ 
                ...
                foreach($notification_list as $noti){
                    ...
                    $list['group_name'] =  $noti['Group']['group_title'];
                    ...
                    $list['message'] =  $noti['Notification']['message'];
					...
					if($noti['Notification']['type']=='E' && $noti['Notification']['group_type']=='F'){
						...
						$list['event_title'] =  $noti['Event']['title'];	
...
;	
	...
	function send_friend_request(){
		...
		$condition_chk_req_sent = "Notification.sender_id = '".$user_id."' AND Notification.receiver_id = '". $receiver_id."' AND Notification.type = 'F' AND Notification.is_receiver_accepted = '0' AND Notification.is_reversed_notification = '0'";
	...
	function my_group_free(){
            ...
            $con_free_groups = "GroupUser.status= '1' AND  GroupUser.user_id = '".$user_id."' AND Group.group_type = 'F'";
            ...
            if(count($arr_free_groups) > 0){
                ...
                foreach($arr_free_groups as $groups){
                    ...
                    $list['title'] =  ucfirst($groups['Group']['group_title']);
	...
	function my_group_business(){
            ...
            $con_business_groups = "GroupUser.status= '1' AND  GroupUser.user_id = '".$user_id."' AND Group.group_type = 'B'";
			$this->GroupUser->bindModel(array('belongsTo' => array('Group' => array('foreignKey' => 'group_id', 'order'=>'Group.group_title ASC'))),false);
    ...
    function send_notification_group_invitation($sender_id=NULL, $receiver_id=NULL, $group_id=NULL, $mode,$page){
		...
		if($user_details['User']['device_token']!='')
		{
           ...
           /* Sajjad Mistri invited you to join Private group AloveradNHH1oizSpk:APA91bF4vfxVTfX30Mj3lJ1H57dwkeoQhgLsRqu3uPzUTzGCtbNOQBhUsjGT0Zf1Eqpgl0Q1Vo6CXFlStL8yOxGERbg3OjB6R7Xfv2K0Js-VSAIPd2F0PJAOrpxmRV_K-ISR4-yxAmdmGroup Joining Invitation To Friends*/
		  	$this->send_push_notification($message,$user_details['User']['device_token'],$page);
...
*/
    ...
    function friend_list_counter(){
        ...
        if(count($friend_detail) > 0){ 
            ...
            foreach($friend_detail as $friends){
                ...
                $conditn = "Chat.to_id = '".$friends['User']['id']."' AND Chat.status = '1'";
    ...
    function chat_list()
    {
        ...
        if(count($ArChatHistory) > 0){ 
            ...
            foreach($ArChatHistory as $ChatHistory){
                ...
                $list['messgae'] = $ChatHistory['Chat']['conversation'];
    ...
    function send_message(){
        ...
        $user_id    = 13;
        $friend_id    = 2;
        $user_name    = "Ashit Ghosh";
        $friend_name    = "Tanima Mukherjee";
        $message = "How are you?";
        ...
        $this->data['Chat']['created'] = date('Y-m-d');
        $this->Chat->create();
