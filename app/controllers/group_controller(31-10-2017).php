  ...
  // $this->_checkSessionUser();
	...
	function submit_email_invitation(){
			...
			if(isset($_REQUEST['mode']) && $_REQUEST['mode']=='invite_emails'){
				...
				for($i=0; $i<count($arr_emails); $i++){
				 ...
				 if(!empty($email_check))
				 {
					...
					if($friendlist >'0')
					{
						...
						$notification_count = $this->Notification->find('count',array('conditions'=>$cond_notification_check));
						...
						if($this->Notification->save($this->data['Notification']))
						{
						   ...
						   $sendCopyTo = '';
							...
							array_push($already_request_sent,$arr_emails[$i]);
						}
			  		}
				}
				 else{
				...
				else if(!empty($already_request_sent)){
			
					$str_already_request_sent_emails= implode(',', $already_request_sent);
					echo "You alreay sent to request to ".$str_already_request_sent_emails;
