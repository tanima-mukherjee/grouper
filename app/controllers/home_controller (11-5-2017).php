...
class HomeController extends AppController {
    ...
    var $name = 'Home';
    var $uses = array('User','SiteSetting','EmailTemplate','State','City','GroupUser','StaticContent','Group','Event','Notification','Friendlist','Testimonial');
...
//echo $created_date;
     ...
     function add_testimonial() {
      ...
      function business_event_calender() 

       {
        ...
        if(!empty($user_id))
        {
        $condition_city_detail = "User.id = '".$user_id."' ";
        $user_city_detail = $this->User->find("first",array('conditions'=>$condition_city_detail));
        $condition = "City.isdeleted = '0' AND City.state_id='".$user_city_detail['User']['state_id']."'";
		$citylist = $this->City->find("all",array('conditions'=>$condition,'fields'=>array('City.id','City.name'),'order' => array('City.name ASC')));
		$this->set('citylist',$citylist); 
		}
		//state_city end//
    ...
    function get_suggestion_list_users(){
			...
			$con_is_req_sent = "Notification.status = '1' AND  Notification.type = 'G' AND Notification.sender_id = '".$user_id."' AND Notification.is_receiver_accepted = '0' AND Notification.is_reversed_notification = '0' ";
