...
class GroupController extends AppController {
    ...
    var $name = 'Group';
    var $uses = array('Category','Group','SubscriptionPlan','User','GroupUser','City','State','GroupImage','GroupDoc','Video','Notification','Friendlist','Event','EmailTemplate');
  ...
  function group_detail_join_now_request($group_id){
   ...
   $this->layout = "";
      ...
      if($is_request > 0)
      {
         ...
         $_SESSION['meesage_type'] = '0';
         $this->redirect("/group/group_detail/".$group_detail['Group']['category_id']);
      }
      else 
      {
  ...
  // $this->_checkSessionUser();
     ...
     function notification_email($notification_id)
        {
             ...
             if($notification_detail['Notification']['type'] == 'P')
            {
              ...
              $message = $notification_detail['Notification']['message'];
	...
	function submit_invitation(){
			...
			if(isset($mode) && $mode =='invite_friends'){
				...
				for($i=0; $i<count($arr_emails); $i++){
					 ...
					 $cond_email_check = "User.email = '".$arr_emails[$i]."' AND User.status = '1'";   // Check whether the email exists
					 $email_check = $this->User->find('first',array('conditions'=>$cond_email_check));
					 if(!empty($email_check))
					 {
							array_push($reject_email_list,$arr_emails[$i]);		//Array contain the emails are already exist
					 }
					 else{
							$this->data['User']['email'] = $arr_emails[$i];
				...
				if(!empty($reject_email_list)){
					...
					echo "Emails have not been sent to ".$str_reject_emails;
				}
				else{
					echo "1";	
				}
	...
	function make_owner(){
		...
		$con_group_dtls = "Group.id = '".$group_id."'";
       	$group_dtls = $this->Group->find("first",array('conditions'=>$con_group_dtls,'fields'=>array('Group.id','Group.group_owners','Group.group_type'))); 
