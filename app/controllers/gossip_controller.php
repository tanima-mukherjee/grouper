...
$from_name = $_POST['from_name'];
 ...
 //$json = array('status' => 0, 'msg'=> 'Enter your message!.');
	...
	$qurGet = $this->Chat->query("select users.image,chats.conversation,chats.created from chats,users where chats.id='".$last_insert_id."' && chats.from_id=users.id");
	
	
	$user_image = $qurGet[0]['users']['image']; 
	$conversation = $qurGet[0]['chats']['conversation']; 
	$created = $qurGet[0]['chats']['created'];
...
$json = '';	 
 ...
 $lid = $_REQUEST['lid'];
	 ...
	 $ArChat = $this->Chat->find('first',array('conditions' => $con, 'fields' => 'Chat.id'));*/
		 ...
		 $qur = $this->Chat->query("select chats.id from chats where (chats.from_id = '".$myid."' AND chats.to_id = '".$fid."') OR (chats.from_id = '".$fid."' AND chats.to_id = '".$myid."') AND chats.status = '1'");
...
function detail_chat_newmsg_bk2(){
$json = '';	
$myid = $_REQUEST['mid'];
$fid = $_REQUEST['fid'];
...
$qur = $this->Chat->query("select chats.from,users.image,chats.conversation,chats.created from chats,users where chats.from_id = users.id AND chats.`to_id` = '".$myid."' AND chats.`from_id` = '".$fid."' AND chats.`status` = '1' order by chats.id desc limit 1");
//print_r($qur);exit;
$lid = $qur[0]['chats']['id'];
$from_name = $qur[0]['chats']['from'];
$user_image = $qur[0]['users']['image'];
$conversation = $qur[0]['chats']['conversation'];
$created = $qur[0]['chats']['created'];
...
$qur = $this->Chat->query("select * from chats where `to_id` = '".$myid."' AND `from_id` = '".$fid."' AND `status` = '1' order by id desc limit 1");
...
$created = $qur[0]['chats']['created'];
 ...
 //while($rw = mysql_fetch_array($qur)){	 
 if(count($qur) > 0){	 
	 $this->data['Chat']['id'] = $lid;
	 $this->data['Chat']['to_id'] = $myid;
	 $this->data['Chat']['from_id'] = $fid;
	 $this->data['Chat']['status'] = '0';
	 $this->Chat->save($this->data['Chat']);
 }
...
function detail_chat_newmsg_bk(){
$json = '';	
 $myid = $_REQUEST['mid'];
 $fid = $_REQUEST['fid'];
