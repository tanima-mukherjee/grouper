...
class EventController extends AppController {
    ...
    function notification_email($notification_id,$eve_name){
     ...
     function event_list()
     {
		...
		if($str_subgroups!=''){
			$event_details = $this->Event->query("SELECT * FROM ( SELECT `id`,`title`,`desc`,`group_type`,`type`,`event_timestamp` as `sort_time`,`group_id`,`status` ,`event_date`,`event_start_date` ,`event_end_date`,`deal_amount`,`location`,`latitude`,`longitude`,`is_multiple_date` FROM `events` WHERE (`group_id` = '".$group_id."' OR (`group_id` IN (".$str_subgroups.") AND `type` = 'public')) AND `status` = '1' AND `event_date` = '".$selectdate."' UNION SELECT `id`,`title`,`desc`,`group_type`,`type`,`event_start_timestamp` as `sort_time`,`group_id`,`status` ,`event_date`,`event_start_date` ,`event_end_date`,`deal_amount`,`location`,`latitude`,`longitude`,`is_multiple_date` FROM `events` WHERE (`group_id` = '".$group_id."' OR (`group_id` IN (".$str_subgroups.") AND `type` = 'public')) AND `status` = '1' AND`event_start_date` <= '".$selectdate."' AND `event_end_date` >= '".$selectdate."') AS `Event`ORDER BY `Event`.`sort_time`ASC");
		}
		else{
